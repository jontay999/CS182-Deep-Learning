...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        ###########################################################################
        dx = np.where(mask, 0.0, dout) / (1.0 - dropout_param['p'])
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
    elif mode == 'test':
            ...
            for hh in range(_h):
                for ww in range(_w):
                    out[n, f, hh, ww] = np.sum(
                        x_padded[n, :, hh * stride:hh * stride + HH, ww * stride:ww * stride + WW] * w[f, :]) + b[f]
    ...
    for n in range(N):
        for f in range(F):
            for hh in range(_h):
                for ww in range(_w):
                    # dw = x * dout, per mask
                    dw[f] += (x_padded[n, :, hh * stride:hh * stride + HH, ww * stride:ww * stride + WW] * dout[
                        n, f, hh, ww])
                    # dx = w * dout, per mask
